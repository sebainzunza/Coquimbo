...
% tided = 'croco_Coq_his_sml.nc'; nc=netcdf(tided,'nowrite'); name='Virtual'; lon=nc{'lon_rho'}(:); ...
%          lat=nc{'lat_rho'}(:); zeta=nc{'zeta'}(:); zeta_m=nc{'zeta'}.scale_factor(:); ...
%         zeta_n=nc{'zeta'}.add_offset(:); ...
%         zeta = zeta*zeta_m + zeta_n; latBeng=-30.18; lonBeng=-71.56;
%         tim=nc{'time'}(:); tim=tim-tim(1); fecha=datetime(2004,1,1)+tim/(24*3600);
tided = 'OS_UH-FDH088_20170628_R.nc'; nc=netcdf(tided,'nowrite'); name='Caldera'; ...
         zeta=nc{'sea_surface_height_above_reference_level'}(:); ...
         ssh = zeta(5:725)./1000; time = nc{'time'}(:);time=time(5:725); ...
         datenumber = time + datenum(1700,01,01,00,00,00);  ...
         fecha = datetime(datenumber,'ConvertFrom', 'datenum');
% tided = 'h088.nc'; name='Caldera'; nc=netcdf(tided,'nowrite'); zeta=nc{'sea_surface_height_above_reference_level'}(:); ...
%         ssh = zeta(5:725); time = nc{'time'}(:); tim = time(5:725); datenumber = tim + datenum(1700,01,01,00,00,00); ...
%         fecha = datetime(datenumber,'ConvertFrom', 'datenum');
...
% saveas(gcf,['Marea_mapa_' name],'png')
...
ax = gca; ax.YLim = [-1 1];
datetick('x','yyyy-mm')
saveas(gcf,['Marea_serie_' name],'png')
...
%
  ...
  title(['Datos ' name])
  saveas(gcf,['Marea_arm_' name],'png')
...
ylabel('NMM_m (m)')
axis tight
ax = gca; ax.YLim = [-0.3 0.3];
datetick('x','yyyy-mm')
saveas(gcf,['Marea_meteo_astro_' name],'png')
